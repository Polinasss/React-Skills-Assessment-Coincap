...
export const ListRender: React.FC<IListRender> = ({ pageNum }) => {
  ...
  const [startPage, setStartPage] = useState(
    (pageNum = Number(location.search.slice(6)))
  );
                ...
                <td>
                  {"$" +
                    new Intl.NumberFormat("de-DE")
                      .format(Number(obj.marketCapUsd))
                      .slice(0, obj.marketCapUsd.length - 23) +
                    "b"}
                </td>
                <td>{"$" + Number(obj.vwap24Hr).toFixed(2)}</td>
                <td>
                  {new Intl.NumberFormat("de-DE")
                    .format(Number(obj.supply))
                    .slice(0, obj.supply.length - 20) + "m"}
                </td>
                <td>
                  {"$" +
                    new Intl.NumberFormat("de-DE")
                      .format(Number(obj.volumeUsd24Hr))
